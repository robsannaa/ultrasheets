  ...
  useEffect(() => {
    ...
    const initializeUniver = async () => {
      ...
      try {
        ...
        const filterLocaleModule = await import("@univerjs/preset-sheets-filter/locales/en-US");
        ...
        try {
          ...
          try {
            ...
            console.log("Filter CSS not found, continuing without it");
          }
        }
        console.log("✅ Filter preset loaded successfully");
      } catch (error) {
        console.warn("⚠️ Filter preset not available, continuing without filters:", error);
      }
      ...
      // EXPOSE univerAPI GLOBALLY for chat component
    ...
    const executeListColumns = async () => {
      ...
      try {
        ...
        for (
          ...
          if (tempColumns.length > columns.length && tempColumns.length >= 2) {
    ...
    const executeCalculateTotal = async (params: any) => {
      ...
      try {
        ...
        for (
          ...
          if (tempColumns.length > headers.length && tempColumns.length >= 2) {
        ...
        console.log(`📊 calculate_total: Found column at index ${columnIndex}`);
          ...
          if (rowData && rowData[columnIndex]) {
            ...
            if (cell && cell.v !== undefined && cell.v !== null && cell.v !== "") {
              ...
              if (cell.f && String(cell.f).includes('SUM(')) {
                existingSumRow = row;
                console.log(`📊 Found existing SUM formula in row ${row + 1}`);
              } else {
        ...
        const dataRange = `${columnLetter}${dataStartRow + 1}:${columnLetter}${
          lastDataRow + 1
        }`;
    ...
    const executeCreatePivotTable = async (params: any) => {
      ...
      try {
        ...
        for (let row = 0; row < Math.min(5, sheetSnapshot.rowCount || 5); row++) {
          const rowData = cellData[row] || {};
          const tempColumns: string[] = [];
        ...
        console.log("📊 executeCreatePivotTable: Available headers:", headers);
        ...
        for (let row = dataStartRow; row < (sheetSnapshot.rowCount || 1000); row++) {
          const rowData = cellData[row];
          if (rowData) {
            const groupValue = rowData[groupByIndex]?.v;
            const numericValue = parseFloat(String(rowData[valueColumnIndex]?.v));
        ...
        const [destCol, destRow] = [destCell.charCodeAt(0) - 65, parseInt(destCell.slice(1)) - 1];
        ...
        headerRange2.setValue(`${aggFunc.toUpperCase()} of ${headers[valueColumnIndex]}`);
        ...
        console.log(`✅ executeCreatePivotTable: Created pivot table at ${destCell}`);
    ...
    const executeGenerateChart = async (params: any) => {
      ...
      const { data_range, chart_type, title, position, width = 400, height = 300 } = params;
      ...
      try {
        ...
        if (!finalDataRange) {
          ...
          finalDataRange = `A${headerRow + 1}:${lastCol}${dataStartRow + dataRowCount}`;
          console.log(`📊 executeGenerateChart: Auto-detected data range: ${finalDataRange}`);
        }
        ...
        console.log(`📊 executeGenerateChart: Creating ${univerChartType} chart with range ${finalDataRange}`);

        // Create improved chart with better data detection
        await createImprovedChart(worksheet, finalDataRange, chart_type, title, chartPosition);
        
        console.log(`✅ executeGenerateChart: Chart created at ${chartPosition}`);

        console.log(`✅ executeGenerateChart: Chart placeholder created at ${chartPosition}`);
    ...
    const executeSwitchSheet = async (params: any) => {
      ...
      return {
        success: false,
        message: "Sheet switching functionality requires proper Univer API implementation",
      ...
      try {
        ...
        if (action === "clear") {
          try {
            ...
            return {
              success: false,
              message: `Failed to clear filters: ${error instanceof Error ? error.message : 'Unknown error'}`
            };
          }
        }
        ...
        if (!filterRange) {
          ...
          if (maxRow > 0 && maxCol > 0) {
            filterRange = `A1:${String.fromCharCode(65 + maxCol)}${maxRow + 1}`;
          } else {
        ...
        if (action === "add") {
          try {
            ...
            console.log("🔍 executeAddFilter: Attempting to use Univer facade API");
            ...
            if (facade && facade.FWorkbook) {
              ...
              if (fWorkbook) {
                ...
                if (fWorksheet) {
                  ...
                  console.log("🔍 executeAddFilter: Got facade range:", fRange2);
                  ...
                  if (typeof (fRange2 as any).createFilter === 'function') {
                    ...
                    console.log("🔍 executeAddFilter: Created filter via facade range");
                    ...
                    if (column && filterValues && filterValues.length > 0) {
                      ...
                      if (/^[A-Z]+$/.test(column.toUpperCase())) {
                        ...
                        for (let col = startCol; col <= endCol; col++) {
                          ...
                          if (cell && cell.v && 
                              String(cell.v).toLowerCase().includes(column.toLowerCase())) {
                            columnIndex = col;
                            break;
                          }
                        }
                      ...
                      if (columnIndex >= 0 && filter && typeof (filter as any).setColumnFilterCriteria === 'function') {
            ...
            if (column && filterValues && filterValues.length > 0) {
              ...
              if (columnIndex >= 0) {
                ...
                for (let row = startRow + 1; row <= endRow; row++) {
                  const rowData = cellData[row];
                  if (rowData && rowData[columnIndex]) {
                    const cellValue = String(rowData[columnIndex].v || '').toLowerCase();
                    const matches = filterValues.some((filterVal: string) => 
                ...
                return {
                  success: true,
                  message: `Filter simulation: Found ${matchingRows} rows matching '${filterValues.join(', ')}' in column ${column}. Note: Visual filtering requires filter UI components.`,
                  range: filterRange,
                  column: column,
                  columnIndex: columnIndex,
                  filterValues: filterValues,
            ...
            throw new Error("Unable to apply filter - filter API not available");

          } catch (filterError) {
            console.error("Filter creation failed:", filterError);
            return {
              success: false,
              message: `Filter operation failed: ${filterError instanceof Error ? filterError.message : 'Unknown error'}. The filter functionality may require additional Univer components.`,
              range: filterRange
            };
          }
    ...
    const createImprovedChart = async (worksheet: any, dataRange: string, chartType: string, title: string, position: string) => {
      try {
        console.log(`📊 createImprovedChart: Creating ${chartType} chart with data range ${dataRange}`);
        
        // Parse position to get row and column coordinates
        const [posCol, posRow] = [position.charCodeAt(0) - 65, parseInt(position.slice(1)) - 1];
        ...
        const chartUrl = await generateImprovedChartUrl(chartData, chartType, title);
        ...
        infoCell.setValue(`${chartType.toUpperCase()}: ${chartData.length - 1} data points`);
    ...
    const extractChartData = async (worksheet: any, range: string) => {
      try {
        ...
        for (let row = startRow; row <= Math.min(endRow, startRow + 100); row++) { // Limit to 100 rows for charts
          const rowData = cellData[row];
          if (rowData) {
            const rowValues = [];
            for (let col = startCol; col <= endCol; col++) {
        ...
        console.log(`📊 extractChartData: Extracted ${extractedData.length} rows of data`);
    ...
    const generateImprovedChartUrl = async (data: any[], chartType: string, title: string) => {
      try {
        ...
        console.log(`📊 generateImprovedChartUrl: Processing ${dataRows.length} data rows with headers:`, headers);
        
        // Validate we have meaningful data
        if (dataRows.length === 0) {
          console.error("❌ generateImprovedChartUrl: No data rows found");
          return `https://quickchart.io/chart?c=${encodeURIComponent(JSON.stringify({ type: 'bar', data: { labels: ['No Data'], datasets: [{ label: 'Error', data: [0] }] } }))}`;
        }
        ...
        const textColumnNames = ['product', 'category', 'region', 'salesperson', 'name', 'item', 'type'];
        ...
        const numericColumnNames = ['total', 'sale', 'amount', 'value', 'price', 'revenue', 'cost'];
        ...
        console.log(`📊 Using column ${labelColumnIndex} (${headers[labelColumnIndex]}) for labels, column ${valueColumnIndex} (${headers[valueColumnIndex]}) for values`);
        ...
        console.log(`📊 Chart data prepared: ${labels.length} categories`, { labels, values: salesData });
        ...
        return `https://quickchart.io/chart?c=${encodeURIComponent(JSON.stringify({ type: 'bar', data: { labels: ['Error'], datasets: [{ data: [0] }] } }))}`;
      }
    };
